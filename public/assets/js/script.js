...
function renderPDF(url, canvasContainer, options) {
  ...
  function renderPage(page) {
    ...
    contex.mozImageSmoothingEnabled = false;
    contex.webkitImageSmoothingEnabled = false;
    contex.msImageSmoothingEnabled = false;
    contex.imageSmoothingEnabled = false;

    contex.save();
    contex.fillStyle = "rgb(255, 255, 255)";
    ...
    // container.style.maxWidth = viewport.width;
    // container.style.maxHeight = viewport.height;
...
document.getElementById("save").addEventListener("click", async function (e) {
  ...
  e.currentTarget.classList.add('save-progress');
  var container = document.getElementById("documentRender");
  var image = document.getElementById("drag");

  interact('#drag').draggable(false);
  interact('#drag').resizable(false);
  ...
  // var jspdf = new window.jsPDF("p", "px", "a4");
  ...
  //   const pdfWidth = jspdf.internal.pageSize.getWidth();
  //   const pdfHeight = jspdf.internal.pageSize.getHeight();
  //   pdfsWidth.push(pdfWidth);
  //   pdfsHeight.push(pdfHeight);
  ...
  //   oldContext.mozImageSmoothingEnabled = false;
  //   oldContext.webkitImageSmoothingEnabled = false;
  //   oldContext.msImageSmoothingEnabled = false;
  //   oldContext.imageSmoothingEnabled = false;

  //   const image = new Image(pdfWidth * 3, pdfHeight * 3);

  //   image.onload = function() {
  //     newContext.drawImage(image, 0, 0, pdfWidth * 3, pdfHeight * 3);
  //   }

  //   image.src = oldCanvas.toDataURL();

  //   jspdf.addImage(image, "PNG", 0, 0, pdfWidth, pdfHeight);
  //   if (index < childsFiltered.length - 1) jspdf.addPage();
  // });

  // console.log(pdfsWidth);
  // console.log(pdfsHeight);
  ...
  $.ajax({
    ...
    if (redirect != '') {
      ...
      window.location.replace('/sent-docs')
    } else {
      console.log(redirect);
      window.location.replace('/mydocuments')
    }
  });
});
